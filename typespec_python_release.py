...
def create_pr_if_needed(repo_path: Path, base_branch: str) -> str | None:
    ...
    result = run_command(
        f'gh pr create --title "[python] release new version" --body "" --base {base_branch}',
        cwd=repo_path
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "repo_path",
        type=str,
        help="Path to the root of the Azure/autorest.python repository"
    )
    parser.add_argument(
        "--base-branch",
        type=str,
        default="main",
        help="The branch to base changes on (default: main)"
    )
    parser.add_argument(
        "--date",
        type=str,
        default=None,
        help="Current date in YYYY-MM-DD format (default: today's date)"
    )
    parser.add_argument(
        "--skip-pr",
        action="store_true",
    ...
    parser.add_argument(
        "--skip-build",
        action="store_true",
        help="Skip the build step (useful for testing)"
    )
